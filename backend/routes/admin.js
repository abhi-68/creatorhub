...
router.get('/email-status', protect, requireRole('admin'), async (req, res) => {
  ...
  res.json({
    ...
    message: ok
      ? 'Mailjet is configured. Use POST /api/admin/email-test to confirm an email actually arrives.'
      : 'Email not configured — set MAILJET_API_KEY, MAILJET_SECRET_KEY, and EMAIL_USER in your Render environment variables.',
  });
});
